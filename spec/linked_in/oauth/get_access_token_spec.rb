...
describe "Get OAuth2 Access Token" do
  ...
  subject { LinkedInAnalytics::OAuth2.new }
  ...
  shared_examples "Success Access Token Fetch" do |*args|
    it "Returns an access token object" do
      VCR.use_cassette("access token success") do
        expect(subject.get_access_token(*args)).to be_kind_of LinkedInAnalytics::AccessToken
      end
    end
    ...
    it "Sets the AcessToken object" do
      VCR.use_cassette("access token success") do
        ...
        expect(subject.access_token).to be_kind_of LinkedInAnalytics::AccessToken
      end
    end
  ...
  shared_examples "Raises InvalidRequest" do |*args|
    it "Raises InvalidRequest" do
      expect{subject.get_access_token(*args)}.to raise_error(LinkedInAnalytics::InvalidRequest, msg)
    end
  ...
  context "When no code is given" do
    let(:msg) {LinkedInAnalytics::ErrorMessages.no_auth_code}
    include_examples "Raises InvalidRequest"
  end

  context "When redirect_uri is not configured and not passed in" do
    before(:example) do
      LinkedInAnalytics.configure { |config| config.redirect_uri = nil }
    end
    let(:msg) {LinkedInAnalytics::ErrorMessages.redirect_uri}
    include_examples "Raises InvalidRequest", code
  end

  context "When redirect_uri is configured and not passed in" do
    before(:example) do
      LinkedInAnalytics.configure { |config| config.redirect_uri = redirect_uri }
    end
    include_examples "Success Access Token Fetch", code
  end
  ...
  context "When redirect_uri does not match previous setting" do
    let(:msg) {LinkedInAnalytics::ErrorMessages.redirect_uri_mismatch}
  ...
  context "When the service is unavailable" do
    ...
    it "raises an OAuthError" do
      VCR.use_cassette("unavailable") do
        expect {subject.get_access_token(code)}.to raise_error(LinkedInAnalytics::OAuthError, msg)
      end
    end
  end
