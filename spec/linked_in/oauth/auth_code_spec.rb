...
describe "OAuth2 Auth Code" do
  ...
  context "When no redirect_uri is given" do
    ...
    let(:err_msg) { LinkedInAnalytics::ErrorMessages.redirect_uri }

    it "Throws an error" do
      expect {subject.auth_code_url}.to raise_error(LinkedInAnalytics::InvalidRequest, err_msg)
    end
  end
  ...
  context "When an auth code url is requested with no options" do
    let(:redirect_uri) { "http://localhost:3000/auth/linkedin/callback" }
    let(:scope) { "r_basicprofile r_emailaddress rw_company_admin w_share" }
    ...
    it "Returns the client id in the uri" do
      expect(params("client_id")).to eq LinkedInAnalytics.config.client_id
    end
  ...
  context "When an auth code url is requested with options" do
    let(:state) { "foobarbaz" }
    let(:scope) { "r_basicprofile rw_company_admin" }
    ...
    it "Returns the client id in the uri" do
      expect(params("client_id", opts)).to eq LinkedInAnalytics.config.client_id
    end
