...
describe LinkedInAnalytics::OAuth2 do
  let(:site)          { LinkedInAnalytics.config.site }
  let(:token_url)     { LinkedInAnalytics.config.token_url }
  let(:authorize_url) { LinkedInAnalytics.config.authorize_url }
  ...
  shared_examples "verify client" do
    it "creates a valid oauth object" do
      expect(subject).to be_kind_of(LinkedInAnalytics::OAuth2)
    end
  ...
  context "When client credentials exist" do
    ...
    context "When custom options are passed in as first arg" do
      subject do
        LinkedInAnalytics::OAuth2.new(options)
      end
    ...
    context "When custom options are passed in" do
      subject do
        LinkedInAnalytics::OAuth2.new(client_id, client_secret, options)
      end
      include_examples "options take"
    end
  ...
  context "When client credentials do not exist" do
    let(:err_msg) { LinkedInAnalytics::ErrorMessages.credentials_missing }
    ...
    it "raises an error" do
      expect { LinkedInAnalytics::OAuth2.new }.to raise_error(LinkedInAnalytics::InvalidRequest, err_msg)
    end
  end

  context "When client credentials are passed in" do
    subject { LinkedInAnalytics::OAuth2.new(client_id, client_secret) }
