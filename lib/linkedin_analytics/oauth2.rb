module LinkedInAnalytics
  # The LinkedInAnalytics::OAuth2::Client class. Inherits directly from [intreda/oauth2](https://github.com/intridea/oauth2)'s `OAuth2::Client`
  #
  # LinkedInAnalytics::OAuth2 sets the following default options:
  #
  # * site = "https://www.linkedin.com"
  # * token_url = "/uas/oauth2/accessToken"
  # * authorize_url = "/uas/oauth2/authorization"
  #
  # More details on LinkedInAnalytics's Authorization process can be found here: https://developer.linkedin.com/documents/authentication
  class OAuth2 < ::OAuth2::Client
    ...
    def initialize(client_id=LinkedInAnalytics.config.client_id,
                   client_secret=LinkedInAnalytics.config.client_secret,
                   options = {}, &block)

      if client_id.is_a? Hash
        options = client_id
        client_id = LinkedInAnalytics.config.client_id
      end
    ...
    #
    # Once a user enters their LinkedInAnalytics credentials, they will be
    ...
    # `redirect_uri` in the `LinkedInAnalytics.configure` block. They can also be
    ...
    #   redirect urls setup on your LinkedInAnalytics Application Settings page.
    ...
    #
    # It also sets the `access_token` field on this LinkedInAnalytics::OAuth2
    # instance.
    #
    # The required `code`
    #
    ...
    #   on this LinkedInAnalytics::OAuth2 instance, then the `redirect_uri` will
    ...
    def get_access_token(code=nil, options={})
      ...
      self.access_token = LinkedInAnalytics::AccessToken.new(tok.token,
                                                    tok.expires_in,
                                                    tok.expires_at)
    ...
    def default_access_code_options(custom_options={})
      ...
      @redirect_uri = LinkedInAnalytics.config.redirect_uri if @redirect_uri.nil?
    ...
    def default_auth_code_url_options(custom_options={})
      ...
      if not LinkedInAnalytics.config.redirect_uri.nil?
        options[:redirect_uri] = LinkedInAnalytics.config.redirect_uri
      end
      if not LinkedInAnalytics.config.scope.nil?
        options[:scope] = LinkedInAnalytics.config.scope
      end
    ...
    def default_oauth_options(custom_options={})
      custom_options ||= {}
      options = {}
      options[:site] = LinkedInAnalytics.config.site
      options[:token_url] = LinkedInAnalytics.config.token_url
      options[:authorize_url] = LinkedInAnalytics.config.authorize_url
